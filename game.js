...
const OSNAME = ['wingdows', 'apple', 'next', 'sgi', 'sun', 'palm',
                'os2', 'bsd', 'linux', 'redhat', 'hurd', 'beos', 'fsystems'];
...
function uiSetPausebutton(action) {
  const el = document.getElementById('pauseHref');
  if (!el) return;
  if (action === 1) {
    el.disabled = false;
    el.textContent = 'Pause Game';
  } else {
    el.textContent = 'Resume Game';
  }
}
...
function uiLoadPix() {
  ...
  handDownImg.src = 'hand_down.png';
  handDownImg.onerror = function() {
    handDownImg = null;
  };
}

function uiLoadAudio() {
  audioMssound = new Howl({src: ['mssound.mp3']});
  AUDIOAHH[0] = new Howl({src: ['ahh0.mp3']});
  AUDIOAHH[1] = new Howl({src: ['ahh1.mp3']});
  AUDIOAHH[2] = new Howl({src: ['ahh2.mp3']});
  AUDIOAHH[3] = new Howl({src: ['ahh3.mp3']});
}
...
// OS
// show OS under cursor (existing) and draw hand cursor
function osDrawCursor() {
  ...
  // draw hand cursor on top (use down image when mouse pressed or bucket grabbed)
  const img = (mouseDown || bucketgrabbed) ? handDownImg : handUpImg;
  if (img && img.complete && img.naturalWidth && img.naturalWidth > 0) {
    // center approx on pointer; tweak offsets if needed
    const w = img.width || 24;
    const h = img.height || 24;
    ctx.drawImage(img, mousex - Math.round(w/2), mousey - Math.round(h/2), w, h);
  }
}
...
function gameUpdate() {
  ...
  case STATE_END:
    ...
    if (scorelistIsHighScore(score)) {
      // Auto-submit as Anonymous instead of prompting
      let name = 'Anonymous';
    ...
    break;
  case STATE_BETWEEN:
    // Advance to next level automatically without alert popup
    gamestate = STATE_PLAYING;
    setupLevel(++level);
    break;
  }
...
function main() {
  ...
  $('canvas').bind('touchend', function(event) {
    // touchend may not have touches; keep current coords
    event.preventDefault();
    mouseDown = false;
    mouseButtonRelease();
  });
